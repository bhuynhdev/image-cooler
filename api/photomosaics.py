...
def average_RGB_for_folder(image_folder_path):
    ...
    for image_file in os.listdir(image_folder_path):
        if image_file.endswith(".jpg"):
            ...
            RGBtuple = str(average_RGB(sourceIm))[
                1:-1]  # Slicing to get rid of ()
            averageRGB.write(RGBtuple + "|" + image_file)
            averageRGB.write("\n")
            print(
                f"Wrote from {image_file}. Size {sourceIm.size}. RGB {RGBtuple}")
    averageRGB.close()
...
def create_cropped_folder(source_images_folder):
    ...
    cropped_images_folder = "./SourceImages/" + \
        os.path.basename(source_images_folder) + "-cropped"
...
def create_photomosaics(inputFile, pixelation_step=10, sourceIm_size=100):
    inputIm = Image.open(inputFile)
    RGB_data_path = create_database("./SourceImages/dog1000")
...
if __name__ == "__main__":
    create_photomosaics("input2.jpg", 10, 100)
